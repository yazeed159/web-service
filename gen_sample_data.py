"""
Generates sample data/trades.json + data/trades/<id>.json in the exact
shape the real n8n pipeline will publish, so the dashboard can be built
and previewed before chart_service.py / n8n are wired up.
"""
import json
import random
import math
...
def make_trade(i, base_dt):
    ...
    for m in range(total_minutes):
        # drift toward entry/exit price to make the chart look intentional
        if m < entry_idx:
            target = entry_price
            frac = m / max(entry_idx, 1)
        elif m < exit_idx:
            target = exit_price
            frac = (m - entry_idx) / max(exit_idx - entry_idx, 1)
        else:
...
def main():
    ...
    for d in range(14):
        ...
        if this_day.weekday() >= 5:
            continue
        n_trades = random.choice([0, 1, 1, 2, 2, 3])

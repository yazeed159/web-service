(function () {
    ...
    els.equitySvg.innerHTML = `
      ...
      <defs>
        <linearGradient id="prGGreen" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#4cbf88" stop-opacity="0.22" />
          <stop offset="100%" stop-color="#4cbf88" stop-opacity="0" />
        </linearGradient>
        <linearGradient id="prGRed" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#e2584f" stop-opacity="0.2" />
          <stop offset="100%" stop-color="#e2584f" stop-opacity="0" />
        </linearGradient>
      </defs>
    `;
  }
  ...
  function fillToMarker(f) {
    return {
      ...
      color: f.side === "buy" ? "#4cbf88" : "#e2584f",
  ...
  function buildPlayChart() {
    ...
    // volume/VWAP/EMA9/EMA20/EMA200 readout that tracks the crosshair the way
    // a broker platform's OHLCV legend does, falling back to the most
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      if (bar) {
        const upDown = bar.color && bar.color.indexOf("76,191,136") !== -1 ? "up" : bar.color && bar.color.indexOf("217,123,63") !== -1 ? "" : "down";
        renderOverlay(
          bar.value, upDown,
          vwapBar ? vwapBar.value : (h ? h.lastVwap : null),
          ema9Bar ? ema9Bar.value : (h ? h.lastEma9 : null),
          ema20Bar ? ema20Bar.value : (h ? h.lastEma20 : null),
          ema200Bar ? ema200Bar.value : (h ? h.lastEma200 : null)
        );
      } else {
        renderOverlay(h ? h.lastVol : null, "", h ? h.lastVwap : null, h ? h.lastEma9 : null, h ? h.lastEma20 : null, h ? h.lastEma200 : null);
      }
    });
    ...
    state.chartHandle = { chart, series, volSeries, vwapSeries, ema9Series, ema20Series, ema200Series, resizeObserver: ro, renderOverlay, lastVol: null, lastVwap: null, lastEma9: null, lastEma20: null, lastEma200: null };
  ...
  function seedSeries(bars) {
    ...
    h.volSeries.setData(bars.map((b) => ({ time: toUnix(b.t), value: b.v, color: b.c >= b.o ? "rgba(76,191,136,0.4)" : "rgba(226,88,79,0.4)" })));
  ...
  function paintFormingBar() {
    ...
    h.volSeries.update({ time: toUnix(bar.t), value: formingVol, color: "rgba(217,123,63,0.4)" });
    ...
    if (bar.ema200 != null) { h.ema200Series.update({ time: toUnix(bar.t), value: bar.ema200 }); h.lastEma200 = bar.ema200; }
    h.renderOverlay(formingVol, "", h.lastVwap, h.lastEma9, h.lastEma20, h.lastEma200);
  }
  function lockInBar() {
    const bar = state.bars[state.barIndex];
    const h = state.chartHandle;
    h.series.update({ time: toUnix(bar.t), open: bar.o, high: bar.h, low: bar.l, close: bar.c });
    h.volSeries.update({ time: toUnix(bar.t), value: bar.v, color: bar.c >= bar.o ? "rgba(76,191,136,0.4)" : "rgba(226,88,79,0.4)" });
    h.lastVol = bar.v;
    h.lastVwap = bar.vwap;
    h.lastEma9 = bar.ema9;
    h.lastEma20 = bar.ema20;
    h.lastEma200 = bar.ema200;

...
(function () {
  ...
  // Dates are computed at click time (a trailing window ending yesterday)
  // so these stay runnable without ever going stale.
  const POPULAR_PRESETS = [
    {
      ...
      // on the first stall bar. Every field below is one this entry_mode
      // + stop_mode combo actually reads -- no donchian/ema/macd/rsi/atr/
      // fixed-stop knobs are set here because orb_breakout + pattern never
      // consult them (see orb_strategy.py); leaving them out means the
      // form shows exactly what will run, nothing decorative.
      ...
      config: {
        ...
        entry_mode: "orb_breakout", orb_minutes: 5, entry_after_orb: true,
    ...
    },
    {
      ...
      // moving: buy the reclaim of a pullback ("dip") to the 9 EMA. Stop
      // is left on stop_mode "pattern" so it sits exactly at that dip
      // candle's Low -- the actual level traders watch -- instead of an
      // arbitrary fixed-cents number that ignores where the dip actually
      // happened. Same reasoning as above: no donchian/macd/rsi/atr/
      // fixed-stop fields, since ema_dip_reclaim + pattern never reads them.
      ...
      config: {
        ...
        starting_capital: 25000, position_sizing_mode: "risk_pct_of_capital", risk_pct_of_capital: 1,
        session_start: "09:30", flatten_time: "15:55", include_commissions: true, slippage_bps: 8,
        entry_mode: "ema_dip_reclaim", ema_period: 9, orb_minutes: 5, entry_after_orb: true,
        stop_mode: "pattern", breakeven_after_cents: 15,
        stall_exit: true,
        scale_in_enabled: true, scale_in_initial_size_pct: 50, scale_in_add_size_pct: 50,
        scale_in_max_adds: 1, scale_in_hold_bars: 2, scale_in_min_gain_cents: 10,
    ...
    },
    {
      ...
      config: {
        ...
        entry_mode: "vwap_reclaim", orb_minutes: 5, entry_after_orb: true,
    ...
    },
    {
      ...
      config: {
        ...
        session_start: "09:30", flatten_time: "15:55", include_commissions: true, slippage_bps: 5,
        entry_mode: "donchian_break", donchian_lookback: 20, orb_minutes: 5, entry_after_orb: true,
        stop_mode: "pattern",
    ...
    },
    {
      ...
      config: {
        ...
        session_start: "09:30", flatten_time: "15:55", include_commissions: true, slippage_bps: 5,
        entry_mode: "rsi_oversold_bounce", rsi_period: 14, rsi_oversold: 30, orb_minutes: 5, entry_after_orb: true,
